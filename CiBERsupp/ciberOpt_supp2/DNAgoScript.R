...
go$found = as.numeric(go$`sig_up_dna_ids.txt (344)`)
...
full_terms_up = ggplot(go, aes(x = fold_enrichment, y =GO_process, color=Padj,size=found)) + 
...
if(!(file.exists('dna_up_terms.pdf'))){
  ggsave('dna_up_terms.pdf',full_terms_up)
